...
public interface CBUAE_BRF16_2_Detail_Repo  extends JpaRepository<CBUAE_BRF16_2_Detail_Entity, String> {
	...
	@Query(value = "select * from CBUAE_BRF16_2_DETAILTABLE where REPORT_DATE=?1 offset ?2 rows fetch next ?3 rows only", nativeQuery = true)
	List<CBUAE_BRF16_2_Detail_Entity> getdatabydateList(Date reportdate,int startpage,int endpage);
	
	@Query(value = "select count(*) from CBUAE_BRF16_2_DETAILTABLE where REPORT_DATE=?1", nativeQuery = true)

...
public interface CBUAE_BRF16_3_Detail_Repo  extends JpaRepository<CBUAE_BRF16_3_Detail_Entity, String> {
	...
	@Query(value = "select * from CBUAE_BRF16_3_DETAILTABLE  ", nativeQuery = true)
	List<CBUAE_BRF16_3_Detail_Entity> getdatabydateList(Date reportdate);
	
	
	@Query(value = "select * from CBUAE_BRF16_3_DETAILTABLE where ROW_ID =?1 and COLUMN_ID=?2 AND REPORT_DATE=?3", nativeQuery = true)
	List<CBUAE_BRF16_3_Detail_Entity> GetDataByRowIdAndColumnId(String rowId,String ColumnId,Date reportdate);
}

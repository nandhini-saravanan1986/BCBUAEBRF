...
import com.bornfire.brf.entities.CBUAE_BRF2_5_Archival_Detail_Entity;
import com.bornfire.brf.entities.CBUAE_BRF2_5_Archival_Detail_Repo;
import com.bornfire.brf.entities.CBUAE_BRF2_5_Archival_Summary_Entity_1;
import com.bornfire.brf.entities.CBUAE_BRF2_5_Archival_Summary_Entity_2;
import com.bornfire.brf.entities.CBUAE_BRF2_5_Archival_Summary_Repo_1;
import com.bornfire.brf.entities.CBUAE_BRF2_5_Archival_Summary_Repo_2;
...
public class CBUAE_BRF2_5_ReportService {
	...
	@Autowired
	CBUAE_BRF2_5_Archival_Detail_Repo BRF2_5_archival_detail_repo;

	@Autowired
	CBUAE_BRF2_5_Archival_Summary_Repo_1 BRF2_5_Archival_Summary_Repo_1;

	@Autowired
	CBUAE_BRF2_5_Archival_Summary_Repo_2 BRF2_5_Archival_Summary_Repo_2;
	...
	public ModelAndView getBRF2_5View(String reportId, String fromdate, String todate, String currency, String dtltype,
		...
		if (type.equals("ARCHIVAL") & version != null) {
			List<CBUAE_BRF2_5_Archival_Summary_Entity_1> T1Master = new ArrayList<CBUAE_BRF2_5_Archival_Summary_Entity_1>();
			List<CBUAE_BRF2_5_Archival_Summary_Entity_2> T1Master1 = new ArrayList<CBUAE_BRF2_5_Archival_Summary_Entity_2>();
			try {
				Date d1 = dateformat.parse(todate);

				T1Master = BRF2_5_Archival_Summary_Repo_1.getdatabydateListarchival(dateformat.parse(todate), version);
				T1Master1 = BRF2_5_Archival_Summary_Repo_2.getdatabydateListarchival(dateformat.parse(todate), version);
	...
	public ModelAndView getBRF2_5currentDtl(String reportId, String fromdate, String todate, String currency,
		...
		Session hs = sessionFactory.getCurrentSession();
		if (type.equals("ARCHIVAL") & version != null) {
			List<CBUAE_BRF2_5_Archival_Detail_Entity> T1Dt1 = new ArrayList<CBUAE_BRF2_5_Archival_Detail_Entity>();
			try {
				Date d1 = dateformat.parse(todate);
				String rowId = null;
				...
				if (rowId != null && columnId != null) {
					T1Dt1 = BRF2_5_archival_detail_repo.GetDataByRowIdAndColumnId(rowId, columnId,
							dateformat.parse(todate), version);

					System.out.println("countavd" + T1Dt1.size());
				} else {
					T1Dt1 = BRF2_5_archival_detail_repo.getdatabydateList(d1, version, currentPage, pageSize);
					totalPages = BRF2_5_archival_detail_repo.getdatacount(dateformat.parse(todate), version);
	...
	public List<Object> getBRF2_5Archival() {
		try {
			...
			merged.addAll(BRF2_5_Archival_Summary_Repo_1.getbrf2_5archival());
			merged.addAll(BRF2_5_Archival_Summary_Repo_2.getbrf2_5archival());
			...
			System.err.println("Error fetching BRF2_5 Archival data: " + e.getMessage());
			e.printStackTrace();
	...
	public byte[] getBRF2_5DetailExcel(String filename, String fromdate, String todate, String currency, String dtltype,
		...
		try {
			...
			if (type.equals("ARCHIVAL") & version != null) {
				byte[] ARCHIVALreport = getBRF2_5DetailExcelARCHIVAL(filename, fromdate, todate, currency, dtltype,
						type, version);
	...
	public byte[] getBRF2_5DetailExcelARCHIVAL(String filename, String fromdate, String todate, String currency,
			String dtltype, String type, String version) {
		try {
			logger.info("Generating Excel for BRF2_5 ARCHIVAL Details...");
			System.out.println("came to Detail download service");
			if (type.equals("ARCHIVAL") & version != null) {
			...
			// Get data
			Date parsedToDate = new SimpleDateFormat("dd/MM/yyyy").parse(todate);
			List<CBUAE_BRF2_5_Archival_Detail_Entity> reportData = BRF2_5_archival_detail_repo
					.getdatabydateList(parsedToDate, version);

			if (reportData != null && !reportData.isEmpty()) {
				int rowIndex = 1;
				for (CBUAE_BRF2_5_Archival_Detail_Entity item : reportData) {
					XSSFRow row = sheet.createRow(rowIndex++);
	...
	public byte[] getBRF2_5Excel(String filename, String reportId, String fromdate, String todate, String currency,
			String dtltype, String type, String version) throws Exception {
		logger.info("Service: Starting Excel generation process in memory.");
		if (type.equals("ARCHIVAL") & version != null) {
			byte[] ARCHIVALreport = getBRF2_5ExcelARCHIVAL(filename, reportId, fromdate, todate, currency, dtltype,
					type, version);
			return ARCHIVALreport;
		}
		List<CBUAE_BRF2_5_Summary_Entity1> dataList = CBUAE_BRF2_5_SUMMARY_REPO1
				.getdatabydateList(dateformat.parse(todate));
		List<CBUAE_BRF2_5_Summary_Entity2> dataList1 = CBUAE_BRF2_5_SUMMARY_REPO2
				.getdatabydateList(dateformat.parse(todate));
...
// dataList1 for entity 2
	...
	public byte[] getBRF2_5ExcelARCHIVAL(String filename, String reportId, String fromdate, String todate,
		...
		List<CBUAE_BRF2_5_Archival_Summary_Entity_1> dataList = BRF2_5_Archival_Summary_Repo_1
				.getdatabydateListarchival(parsedDate, version);
		logger.info("Service: dataList (Repo 1) size: {}", dataList.size());

		List<CBUAE_BRF2_5_Archival_Summary_Entity_2> dataList1 = BRF2_5_Archival_Summary_Repo_2
				.getdatabydateListarchival(parsedDate, version);
		logger.info("Service: dataList1 (Repo 2) size: {}", dataList1.size());

		if (dataList.isEmpty() && dataList1.isEmpty()) {
			logger.warn(
					"Service: No data found in both BRF2_5 summary repositories for report_date={} and version={}. Returning empty result.",
					parsedDate, version);
			return new byte[0];
		}
		String templateDir = env.getProperty("output.exportpathtemp");
		...
		try (InputStream templateInputStream = Files.newInputStream(templatePath);
			...
			if (!dataList.isEmpty()) {
				for (int i = 0; i < dataList.size(); i++) {
					...
					CBUAE_BRF2_5_Archival_Summary_Entity_1 record = dataList.get(i);
					System.out.println("rownumber=" + startRow + i);
					Row row = sheet.getRow(startRow + i);
					if (row == null) {
						row = sheet.createRow(startRow + i);
...
// dataList1 for entity 2
				if (!dataList1.isEmpty()) {
					for (int i = 0; i < dataList1.size(); i++) {
						CBUAE_BRF2_5_Archival_Summary_Entity_2 record1 = dataList1.get(i);
